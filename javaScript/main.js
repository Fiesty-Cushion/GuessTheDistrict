...
function check(userChoice) {
  if (!answeredAlready) {
    if (userChoice == storesRandomDistricts[pointerRandomDistricts - 1]) {
      ...
      if (
        storesRandomDistricts.length != 77 &&
        pointerRandomDistricts == indexSRD
      ) {
        randomDistrict();
      }
      ...
      answeredIncorrectly.push(
        storesRandomDistricts[pointerRandomDistricts - 1]
      );
      streak++;
...
function previousDistrict() {
  ...
  if (pointerRandomDistricts > 1) {
    ...
    document.getElementById(
      "counter"
    ).innerText = `(${pointerRandomDistricts}/77)`;
  }
}
